                  ...
                  <div class="x_title">
                    ...
                    <form class="form-horizontal form-label-left" method="POST" action="" novalidate>
                      ...
                      if(isset($_SESSION['id'])){
                        $res=mysqli_query($con,"SELECT `faculty`.`user_id`, `faculty`.`name`, `faculty`.`address`, `faculty`.`date_of_birth`, `faculty`.`phone_no`, `faculty`.`qualification`, `faculty`.`experience`, `faculty`.`join_date`, `faculty`.`last_log_in`, `users`.`email_id` FROM `faculty` JOIN `users` ON `faculty`.`user_id`=`users`.`user_id` WHERE `faculty`.`user_id`='".$_SESSION['id']."' AND `users`.`user_type`= 'Faculty'");

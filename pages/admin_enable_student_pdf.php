...
$res=mysqli_query($con,"SELECT `student`.`reg_no`, `student`.`roll_no`, `student`.`name`, `student`.`address`, `student`.`phone_no`, `student`.`sem`, `student`.`department`,  `users`.`email_id` FROM `student` JOIN `users` ON `student`.`user_id` = `users`.`user_id` WHERE `users`.`user_type`= 'Student' and `users`.`user_status`='0' ORDER BY `department`, `sem` ASC");

...
class PDF extends FPDF
{
  function Header()
  {
      ...
      $this->Cell(60);
      // Title
      $this->Cell(70,10,'DISABLED FACULTY LIST',1,0,'C');
      // Line break
      $this->Ln(20);
  }
  function Footer()
...
$res=mysqli_query($con,"SELECT `faculty`.`name`, `faculty`.`address`, `faculty`.`phone_no`, `faculty`.`department`, `users`.`email_id` FROM `faculty` JOIN `users` ON `faculty`.`user_id` = `users`.`user_id` WHERE `users`.`user_type`= 'Faculty' and `users`.`user_status`='1' ORDER BY `department` ASC");
